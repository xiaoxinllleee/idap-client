...
export const JeecgListMixin = {
  ...
  methods: {
    ...
    searchQueryByUrl(url) {
      this.beforeSearchQuery(url)
      this.loadDataByUrl(1, url)
    },
    beforeSearchQuery () {
    ...
    handleExportXlsByNameAndUrl (fileName, url) {
      if (!fileName || typeof fileName != 'string') {
        fileName = '导出文件'
      }
      let param = this.getQueryParams();
    ...
    handleExportXlsByUrl () {
      ...
      console.log('导出参数', param)
      this.exportLoading = true
      getAction(this.url.dcxxcjb, param).then((res) => {
        let idKey = { 'idKey': res.message }
        getAction(this.url.zipUrl).then((res) => {
          if (res.success) {
            window.open(
    ...
    expComonTemp(val){
      /* 下载指定目录下的文件*/
      window.location.href = `${window._CONFIG['staticDomainURL']}/excle/temp/` + val
    }
  }
