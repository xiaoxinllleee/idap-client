<template>
  <a-card :bordered="false">
      ...
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="监控建议">
              <a-select v-model="queryParam.jkjy" @change="updete">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="监控状态">
              <a-select  v-model="queryParam.jkzt" @change="updete">
                <a-select-option value="1">全部</a-select-option>
        ...
        <template v-if="toggleSearchStatus">
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="首次高危">
              <a-select  v-model="queryParam.scgw" @change="updete">
                <a-select-option value="1">全部</a-select-option>
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="是否分配">
              <a-select  v-model="queryParam.sffp" @change="updete">
              <a-select-option value="1">全部</a-select-option>
              <a-select-option value="2">是</a-select-option>
              <a-select-option value="3">否</a-select-option>
              </a-select>
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="列表排序">
              <a-select  v-model="queryParam.lbpx" @change="updete">
                ...
                <a-select-option value="3">监控建议</a-select-option>
                <a-select-option value="4">监控创建时间</a-select-option>
                <a-select-option value="5">批次号</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="创建时间">
              <a-range-picker :placeholder="['开始日期', '结束日期']" v-model="queryParam.cjsj" :format="DateFormat"
                              @change="onCjsjDateChange" style="width: 100%;" :default-value="[moment(new Date(), 'YYYY-MM-DD'), undefined]"/>
            </a-form-item>
          </a-col>
    ...
    <div class="table-operator">
      <a-button style='background-color: #f7bf67;color: white;margin-left: 10px'   @click="initData">新增单个客户监控</a-button>
      <a-button style='background-color: #f7bf67;color: white;margin-left: 10px'  @click="plAdd" >批量新增监控</a-button>
      <a-button style='background-color: #f7bf67;color: white;margin-left: 10px' @click='down' >终止监控</a-button>
      <a-button style='background-color: #f7bf67;color: white;margin-left: 10px'  @click='ycjkjssj' >延长监控结束时间</a-button>
      <a-button style='background-color: #f7bf67;color: white;margin-left: 10px'  :loading="exportLoading" @click="down2('贷前风险识别')">导出监控信息</a-button>
      <a-button style='background-color: #f7bf67;color: white;margin-left: 10px'  :loading="exportLoading" @click="down3('贷前风险识别')">导出监控数据信息</a-button>
      ...
      <a-modal
        ...
        <a-form :form="form">
          <a-spin :spinning="confirmLoading">
            <a-row>
              <a-col :lg="8">
                <a-form-item
                  ...
                  :wrapperCol="wrapperCol"
                  label="操作账号">
                  <a-select  v-model="queryParam.czzh">
                    <a-select-option value="1">当前用户和下级子账号</a-select-option>
                    <a-select-option value="2">当前用户</a-select-option>
                    <a-select-option value="3">下级子账号</a-select-option>
                  </a-select>
            ...
            <a-row>
              <a-col :lg="8">
                <a-button type="primary" style="background-color: #f7bf67;" @click="pljssj">批量延长监控结束时间</a-button>
              </a-col>
            </a-row>
  ...
  computed: {
    ...
    methods: {
      ...
      searchReset() {
        ...
        this.queryParam.jkjy = '1'
        this.queryParam.jkzt = '1'
        this.queryParam.scgw = '1'
        this.queryParam.sffp = '1'
      ...
      down3() {
        ...
        that.$confirm({
          title: '系统消息',
          okText: '确定',
          content: '系统默认只导出近三个月数据,确定导出报告数据信息吗？'
        })
      },
      pljssj() {
        let that = this
