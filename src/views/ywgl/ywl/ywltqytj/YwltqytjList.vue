<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button :loading="exportLoading" icon="download" @click="handleExportXls('业务量提取与统计')" v-has="'ywltqytj:exportXls'">导出</a-button>
      <a-button icon="area-chart" @click="init" v-has="'ywltqytj:init'">业务提取</a-button>
    ...
    <!-- 表单区域 -->
    <ywltqytj-modal ref="modalForm" @ok="modalFormOk"></ywltqytj-modal>
  </a-card>
</template>

<script>
  import YwltqytjModal from './modules/YwltqytjModal'
  ...
  export default {
    ...
    components: {
      YwltqytjModal,JTreeSelect
    },
    data () {
      return {
        description: '业务量提取与统计管理页面',
        defaultMonthOpera: '',
        monthFormat: 'YYYY-MM',
        // 表头
        columns: [
		...
		url: {
          list: "/ywltqytj/ywltqytj/list",
          delete: "/ywltqytj/ywltqytj/delete",
          deleteBatch: "/ywltqytj/ywltqytj/deleteBatch",
          exportXlsUrl: "ywltqytj/ywltqytj/exportXls",
          importExcelUrl: "ywltqytj/ywltqytj/importExcel",
          init: "/ywltqytj/ywltqytj/init",
       },
    }
  },
  computed: {
    ...
    methods: {
      init() {
        if (this.queryParam.tjyf == '' || this.queryParam.tjyf == undefined || this.queryParam.tjyf=='') {
          this.$message.warning('统计月份不能为空！')
          return
        }
        if (this.queryParam.zzbz==null || this.queryParam.zzbz==undefined || this.queryParam.zzbz==''){
          this.$message.warning('组织标志不能为空！')
          return
        }
        this.queryParam.tjyf= moment(this.queryParam.tjyf).format("YYYY-MM") + "-01"
        this.loading = true
        getAction(this.url.init, {'tjyf': this.queryParam.tjyf, 'zzbz': this.queryParam.zzbz}).then((res) => {
          if (res.success) {
