<template>
  <a-modal :title="title"
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <!--<a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="区域代码">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/pjsjxsjxarea/pjsxSjxArea/add",
          edit: "/pjsjxsjxarea/pjsxSjxArea/edit",
        },
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'qydm','sjxid','sjxmc','sjxwd','status','zxsx','sjly','bz','sjxsql','khlx'))
        });
      },
      onChose(checked) {
        if (checked) {
