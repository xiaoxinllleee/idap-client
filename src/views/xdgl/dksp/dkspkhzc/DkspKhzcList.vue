<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button @click="handleAdd" type="primary" icon="plus" v-has="'DkspKhzc:add'">注册</a-button>
      <a-button icon="download" @click="handleExportXls('贷款审批客户注册')" v-has="'DkspKhzc:exp'">导出</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <!--未提交或者驳回可以编辑-->
          <a v-if="record.result == '0' || record.result == '3' ||  !record.processId" @click="handleEdit(record)" v-has="'DkspKhzc:edit'">编辑</a>
          <a v-else @click="handleView(record)" v-has="'DkspKhzc:view'">查看</a>
          <a-divider type="vertical" v-if="record.processId && record.procInstId"/>
          <a v-if="record.processId && record.procInstId" @click="historyDetail(record)" v-has="'DkspKhzc:spls'">审批历史</a>
          <a-divider type="vertical" v-if="record.processId && record.procInstId"/>
          <a v-if="record.processId && record.procInstId && record.result == '2'" @click="historyDownload(record)" v-has="'DkspKhzc:spyjb'">审批意见表</a>
    ...
    <!-- 表单区域 -->
    <dkspKhzc-modal ref="modalForm" @ok="modalFormOk"></dkspKhzc-modal>
    <!--审批历史 -->
...
<script>
  import DkspKhzcModal from './modules/DkspKhzcModal'
  ...
  export default {
    ...
    components: {
      DkspKhzcModal, JTreeSelect, historicDetail, JInput
    },
    data () {
      return {
