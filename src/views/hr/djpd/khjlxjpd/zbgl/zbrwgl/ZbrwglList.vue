<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button @click="handleAdd" type="primary" icon="plus" v-has="'Zbrwgl:add'">新增</a-button>
      <a-button icon="download" @click="handleExportXls('指标任务管理')" v-has="'Zbrwgl:exp'">导出</a-button>
      <a-button icon="import" @click="excelImport" v-has="'Zbrwgl:imp'">导入</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-has="'Zbrwgl:edit'">编辑</a>
    ...
    <!-- 表单区域 -->
    <zbrwgl-modal ref="modalForm" @ok="modalFormOk"></zbrwgl-modal>
    <zbrwgl-add-modal ref="modalAddForm" @ok="modalFormOk"></zbrwgl-add-modal>
  </a-card>
</template>

<script>
  import ZbrwglModal from './modules/ZbrwglModal'
  import ZbrwglAddModal from './modules/ZbrwglAddModal'
  ...
  export default {
    ...
    components: {
      ZbrwglModal,JTreeSelect,filterObj,ExcelImport,ZbrwglAddModal
    },
    data () {
      return {
        description: '指标任务管理管理页面',
        dateValue: '',
        pdzqD: null,
        pdzqM: null,
        pdzqQ: null,
		...
		url: {
          list: "/zbrwgl/zbrwgl/list",
          delete: "/zbrwgl/zbrwgl/delete",
          deleteBatch: "/zbrwgl/zbrwgl/deleteBatch",
          exportXlsUrl: "zbrwgl/zbrwgl/exportXls",
          importExcelUrl: "zbrwgl/zbrwgl/importExcel",
          exportTemplateUrl: "/zbrwgl/zbrwgl/exportTemplateXls",
       },
        exportTemplateName:'指标任务管理管理导入模板'
    }
  },
  computed: {
    ...
    methods: {
      handleDelete: function (record) {
        ...
        var that = this;
        deleteAction(that.url.delete, {pdzq: record.pdzq,pdrq: record.pdrq,zzbz: record.zzbz,
          gwbz: record.gwbz,yggh: record.yggh,zbid: record.zbid}).then((res) => {
          if (res.success) {
      ...
      // 查询条件[pdrq]格式化Func
      getQueryParams() {
        console.log('ZbrwglList => getQueryParams()')
      ...
      handleAdd: function () {
        ...
        var param=Object.assign({},this.queryParam,this.isorter);
        param.pdrq=pdrqMoment;
        console.log(param.gwbz)
        if (param.pdzq == undefined){
        ...
        if (param.zzbz  == undefined || param.zzbz ==''){
          this.$message.error("请选择组织标标识!")
          return
        }
        if (param.gwbz == undefined || param.gwbz =='undefined'){
          this.$message.error("请选择岗位标识!")
          return
        }
        this.$refs.modalAddForm.add(param);
        this.$refs.modalAddForm.title = "新增";
      },
