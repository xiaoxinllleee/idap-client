        ...
        httpAction(this.url.importExcelUrl,formData, method).then((res)=>{
          if(res.success){
            for (let i=0;i<res.result.length;i++){
              let info={}
              info.yggh=res.result[i].yggh
              info.zzbz=res.result[i].xwgsszh
              this.otherInfo.push(info)
            }
            httpAction("/pqqxgl/yxdyglPqqxgl/addPqqxgl",this.otherInfo,'post').then((response)=>{
              if (response.success){
                this.$message.success(res.message);
                this.$emit('ok', res.result.filePath);
              }
            })
          }else{
