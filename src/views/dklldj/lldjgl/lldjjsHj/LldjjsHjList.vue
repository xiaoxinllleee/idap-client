<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button type="primary" @click="handleAdd" icon="plus" :loading="addLoading" v-has="'lldjjsHj:add'">{{this.addBtnText}}</a-button>
      <a-button icon="download" @click="handleExportXls('贷款利率定价计算信息')" v-has="'lldjjsHj:exportXls'">导出</a-button>
    </div>
    ...
    <div>
      <a-table ref="lldjjsTable"
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-has="'lldjjsHj:edit'">重新计算</a>
          <a-divider type="vertical"/>
          <a @click="print(record,'查看','view')" v-has="'lldjjsHj:view'">查看</a>
          <a-divider type="vertical" v-if="record.spzt != 1"/>
          <a @click="confirm(record)" v-if="record.spzt != 1" v-has="'lldjjsHj:confirm'">确认</a>
          <a-divider type="vertical" v-if="record.spzt == 1"/>
          <a @click="print(record,'打印','print')" v-if="record.spzt == 1" v-has="'lldjjsHj:print'">打印</a>
          <a-divider type="vertical"/>
           <a-popconfirm title="确定删除吗?" @confirm="() => handleDelete(record)" v-has="'lldjjsHj:delete'">
            <a>删除</a>
          </a-popconfirm>
        </span>
      </a-table>
    ...
    <!-- 表单区域 -->
    <lldjjs-hj-modal ref="modalForm" @ok="ok"></lldjjs-hj-modal>
    <lldjjs-hj-confirm ref="confirmForm" @ok="ok"></lldjjs-hj-confirm>
  </a-card>
</template>
...
<script>
  ...
  import LldjjsHjModal from "@/views/dklldj/lldjgl/lldjjsHj/modules/LldjjsHjModal";
  import LldjjsHjViewPrint from "@/views/dklldj/lldjgl/lldjjsHj/modules/LldjjsHjViewPrint";
  import LldjjsHjConfirm from "@/views/dklldj/lldjgl/lldjjsHj/modules/LldjjsHjConfirm";
  ...
  export default {
    ...
    components: {
      ...
      JTreeSelect,
      JDictSelectTag,
      LldjjsHjModal,LldjjsHjViewPrint,LldjjsHjConfirm
    },
    data() {
      return {
        description: '利率定价计算管理页面（洪江）',
        dateValue: '',
        djnf: '',
        addBtnText: '添加',
        addLoading: false,
        ...
        url: {
          list: "/dklldj/lldjjsHj/list",
          add: "/dklldj/lldjjsHj/add",
          edit: "/dklldj/lldjjsHj/edit",
          delete: "/dklldj/lldjjsHj/delete",
          confirm: "/dklldj/lldjjsHj/confirm",
          viewPrint: "/dklldj/lldjjsHj/viewPrint",
          exportXlsUrl: "dklldj/lldjjsHj/exportXls",
        },
      }
    },
    computed: {},
    ...
    methods: {
      ...
      handleAdd: function () {
        ...
        getAction(this.url.add, param).then((res) => {
          if (res.success) {
            ...
            this.$refs.modalForm.add(res.result.djsqxx)
          } else {
      ...
      confirm(record) {
        ...
        var djid = record.djid
        if (!djid) {
          notification.warning({message: '系统提示', description: '定价为空，请核实！', duration: 4})
          return
        }
        var spzt = Number.parseInt(record.spzt)
        if (spzt === 1) {
        ...
        postAction(that.url.confirm, params).then((res) => {
          if (res.success) {
            ...
            if (res.result != null) {
              ...
              that.$set(that.confirmdata,'zxll',res.result.zxll == null ? 0.0000 : Number(res.result.zxll).toFixed(4))
              that.$confirm({
                ...
                onOk() {
                  console.log('用户确认操作！');
                  that.$refs.confirmForm.title = "利率定价确认";
                  that.$refs.confirmForm.disableSubmit = false
                  that.$refs.confirmForm.edit(record);
                },
                onCancel() {
                  console.log('用户取消操作！');
                },
