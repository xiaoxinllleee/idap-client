<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="8">
            <a-form-item label="贷款期限">
              <j-dict-select-tag placeholder="请选择" v-model="queryParam.dkqx" dictCode="dkqxhj"/>
            </a-form-item>
          </a-col>
    ...
    <div class="table-operator">
      <a-button type="primary" @click="handleAdd" icon="plus" :loading="addLoading" v-has="'lldjsqHj:add'">{{this.addBtnText}}</a-button>
      <a-button @click="handleExportXls('贷款利率定价申请信息')" icon="download" v-has="'lldjsqHj:exportXls'">导出</a-button>
    ...
    <div>
      <a-table ref="table"
        ...
        <span slot="dbfs" slot-scope="dbfs">
          <a-tag :key="dbfs" :color="dbfs==='信用'?'#FB6241':dbfs==='保证'?'#8A2BE2':dbfs==='抵（质）押'?'#01D867':'grey'">
            {{ dbfs }}
          </a-tag>
        </span>
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-has="'lldjsqHj:edit'">编辑</a>
          <a-divider type="vertical" />
          <a-popconfirm title="确定删除吗?" @confirm="() => handleDelete(record)" v-has="'lldjsqHj:delete'">
            <a>删除</a>
          </a-popconfirm>
        </span>
    ...
    <!-- 表单区域 -->
    <lldjsq-hj-modal ref="modalHjForm" @ok="modalFormOk"></lldjsq-hj-modal>
  </a-card>
</template>

<script>
  import LldjsqHjModal from './modules/LldjsqHjModal'
  ...
  export default {
    ...
    data() {
      return {
        ...
        djnf: '',
        url: {
          list: "/dklldj/lldjsqHj/list",
          addDjsq: "/dklldj/lldjsqHj/addDjsq",
          editDjsq: "/dklldj/lldjsqHj/editDjsq",
          delete: "/dklldj/lldjsqHj/delete",
          exportXlsUrl: "/dklldj/lldjsqHj/exportXls",
        },
      }
    },
    computed: {},
    ...
    methods: {
      ...
      handleAdd() {
        ...
        getAction(this.url.addDjsq, params).then((res) => {
          if (res.success) {
            ...
            this.$refs.modalHjForm.title = "添加：贷款利率定价申请"
            this.$refs.modalHjForm.opration = 'add'
            this.$refs.modalHjForm.edit(record)
          } else {
       ...
       */
      handleEdit(record) {
        // if (record.zzbz === undefined || record.zzbz === "") {
        //   notification.warning({message: '系统提示', description: '所属组织为空，请核实!', duration: 4})
        //   return false
        // }
        console.log('待修改的定价申请信息\n',record)
        ...
        getAction(this.url.editDjsq, data).then((res) => {
          if (res.success) {
            ...
            this.$refs.modalHjForm.title = "修改：贷款利率定价申请"
            this.$refs.modalHjForm.opration = 'edit'
            this.$refs.modalHjForm.edit(record)
          } else {
            notification.warning({ message: '系统提示', description: res.message,duration: 4})
          }
