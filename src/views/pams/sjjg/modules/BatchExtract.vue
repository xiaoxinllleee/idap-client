...
<script>
  import {jxextractBatch,updateBatchzt} from '@/api/api'
      ...
      doExec() {
        ...
        updateBatchzt({ksrq:that.tjrqb.format("YYYYMMDD"),jsrq:that.tjrqe.format("YYYYMMDD"),rwid: that.rowData.id}).then((res) => {
          if (res.success) {
            ...
            jxextractBatch({spname:that.rowData.rwgc,ksrq:that.tjrqb.format("YYYYMMDD"),jsrq:that.tjrqe.format("YYYYMMDD"),rwid:that.rowData.id}).then((res) => {
              if (res.success) {
                  notification.open({
                    type:'success',
                    message: '提取成功',
