<template>
  <a-modal
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-row :gutter="20" type="flex" >
          ...
          <a-col :span="18">
            <a-button @click="FindBack(1)" type="primary" icon="plus" style="margin-bottom: 5px;">添加指标</a-button>
            <j-editable-table
    ...
    <account-area ref="accountZbid" @close="colseArea"></account-area>
  </a-modal>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      handleOk () {
        ...
        if (this.value == 'undefined' ||this.value == undefined || this.value.length === 0){
          this.$message.warning('岗位名称不能为空！')
          return
        }
        this.$refs.aljcGwSetTable.getValues((error, values) => {
          // 错误数 = 0 则代表验证通过
          if (error === 0) {
            ...
            const that = this;
            httpAction('/khfagl/erpAssessAljc/add',formData,'post').then((res)=>{
              if(res.success){
