    ...
    methods: {
      ...
      handleOk () {
        ...
        if (this.model.jfz == 'undefined' ||this.model.jfz == undefined){
          this.$message.warning('计分制不能为空！')
          return
        }
        this.$refs.phjfkGwSetTable.getValues((error, values) => {
          // 错误数 = 0 则代表验证通过
          if (error === 0) {
            ...
            const that = this;
            httpAction('/khfagl/erpAssessPhjfk/GwSave',formData,'post').then((res)=>{
              if(res.success){
