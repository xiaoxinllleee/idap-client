...
<script>
  ...
  export default {
    ...
    components:{JTreeSelect,JTreeSelectNotJg},
    ...
    methods: {
      ...
      handleOk() {
        let _this=this;
        if(_this.gswg==null||_this.gswg==""){
          _this.$message.warning('请选归属网格！');
        }else{
          _this.$confirm({
            title: '确认分配客户网格?',
            content: h => <div style="color:red;">分配网格客户数:{_this.selectedRowKeys.length}</div>,
            onOk() {
              putAction("/dfpkh/dfpWgkh/wgfpAction",{"rows":_this.dataSource2,"gswg":_this.gswg}).then((res)=>{
                if(res.success){
