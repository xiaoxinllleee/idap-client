<template>
  <a-card :bordered='false'>
        ...
        <a-row :gutter='24'>
          ...
          <a-col :xl='6' :lg='7' :md='8' :sm='24'>
            <a-form-item label='任务类型'>
              <j-dict-select-tag placeholder='请选择任务类型' v-model='queryParam.rwlx' allowClear
                                 dict-code='marketing_type1'></j-dict-select-tag>
            </a-form-item>
          </a-col>
          <a-col :xl='6' :lg='7' :md='8' :sm='24'>
    ...
    <div class='table-operator'>
      <a-button icon='download' :loading='exportLoading' @click="handleExportXls('支行任务完成情况统计表')">导出
      </a-button>
    </div>
    ...
    <div>
      <a-table
        ...
        <span slot='dryxsb' slot-scope='dryxsb,record'>
          <a @click="detailInfo(record,'3',dryxcg)">{{ dryxsb }}</a>
         </span>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/rwmcmxzh/tjfxTaskWcmxZh/list',
        exportXlsUrl: '/rwmcmxzh/tjfxTaskWcmxZh/exportXls',
        initData: '/rwmcmxzh/tjfxTaskWcmxZh/initData'
      },
      maxDate: ''
    }
  },
  ...
  methods: {
    getMaxDate() {
      getAction('/rwmcmxzh/tjfxTaskWcmxZh/getMaxDate', null).then(res => {
        if (res.success) {
