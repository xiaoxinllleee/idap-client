<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button :loading="exportLoading" icon="download" @click="handleExportXls('自助设备业务量')"
                v-has="'zzsbywl:exportXls'">导出
      </a-button>
      <a-button icon="area-chart" @click="init" v-has="'zzsbywl:init'">提取</a-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: "/zzsbywl/zzsbywl/list",
          delete: "/zzsbywl/zzsbywl/delete",
          deleteBatch: "/zzsbywl/zzsbywl/deleteBatch",
          exportXlsUrl: "zzsbywl/zzsbywl/exportXls",
          importExcelUrl: "zzsbywl/zzsbywl/importExcel",
          init: "/zzsbywl/zzsbywl/init",
        },
      }
    },
    computed: {
    ...
    methods: {
      getCurrentMonth() {
        ...
        this.queryParam = {tjwd: 'MM', tjyf: undefined};
        this.queryParam.tjyf  = moment(curDate, 'YYYY-MM');
      },
      init() {
        console.log('ZzsbywlList => init()')
        let tjyfStr
        let tjwdStr
      ...
      selectValue(value, date) {
        console.info('selectValue-value,date',value,date)
        let val = value;
      ...
      getQueryParams() {
        ...
        if (parm.tjwd == 'MM' && parm.tjyf != '') {
          tjyfMoment = moment(parm.tjyf).format("YYYY-MM") + '-01';
        }
        if (parm.tjwd == 'Q' && this.jdssnf != '' && this.tjjd != '') {
          let day = new Date(this.jdssnf, this.tjjd, 0)
          let dayCount = day.getDate()
          tjyfMoment = this.jdssnf + '-' + this.tjjd + '-01';
        }
        if (parm.tjwd == 'YYYY' && this.tjnf != '') {
          tjyfMoment = this.tjnf + "-01-01";
        }
