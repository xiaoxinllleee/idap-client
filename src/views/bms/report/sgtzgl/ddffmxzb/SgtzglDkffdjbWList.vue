...
<!--      <a-button @click="handleDelete1" icon="delete">根据日期删除</a-button>-->
    ...
    <!-- 表单区域 -->
    <sgtzgl-dkffdjb-w-modal ref="modalForm" @ok="modalFormOk"></sgtzgl-dkffdjb-w-modal>
  </a-card>
</template>

<script>
import SgtzglDkffdjbWModal from './modules/SgtzglDkffdjbWModal'
...
export default {
  name: "SgtzglDkffdjbWList",
  mixins: [JeecgListMixin],
  components: {
    SgtzglDkffdjbWModal, JTreeSelect, ExcelImport
  },
  data() {
    return {
      description: '贷款发放登记簿（周）管理页面',
      // 表头
      ...
      url: {
        list: "/sgtzgl/dkffdjbW/list",
        delete: "/sgtzgl/dkffdjbW/delete",
        deleteBatch: "/sgtzgl/dkffdjbW/deleteBatch",
        deleteByBatch: "/sgtzgl/dkffdjbW/deleteByBatch",
        exportXlsUrl: "sgtzgl/dkffdjbW/exportXls",
        importExcelUrl: "/sgtzgl/dkffdjbW/importExcel",
        exportTemplateUrl: "/sgtzgl/dkffdjbW/exportTemplateXls",
      },
      exportTemplateName: '贷款发放登记簿（周）导入模板'
    }
  },
  computed: {
  ...
  methods: {
    handleDelete1: function () {
      ...
      if (this.queryParam.fiscalDate == undefined && this.queryParam.fiscalDate == null) {
        ...
        this.rq = moment(this.queryParam.fiscalDate).day(1).format('YYYYMMDD');
        console.log(this.rq)
        var that = this;
