<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="入库日期">
              <a-date-picker placeholder="请输入入库日期" v-model="queryParam.rkrq"></a-date-picker>
            </a-form-item>
          </a-col>
          <template v-if="toggleSearchStatus">
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="出库日期">
                <a-date-picker placeholder="请输入出库日期" v-model="queryParam.ckrq"></a-date-picker>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div class="table-operator">
      ...
      <a-button icon="download" :loading="exportLoading" @click="handleExportXls('担保品基本信息登记簿')">导出
      </a-button>
    ...
    <!-- 表单区域 -->
    <sgztDbpjbxxdjb-modal ref="modalForm" @ok="modalFormOk"></sgztDbpjbxxdjb-modal>
    <excel-import ref="excelImportModal" @ok="importComplete"/>
  </a-card>
</template>

<script>
import SgztDbpjbxxdjbModal from './modules/SgztDbpjbxxdjbModal'
...
export default {
  name: "SgztDbpjbxxdjbList",
  mixins: [JeecgListMixin],
  components: {
    SgztDbpjbxxdjbModal, ExcelImport,JTreeSelect
  },
  data() {
    return {
      description: '担保品基本信息登记簿管理页面',
      // 表头
      ...
      columns: [
        ...
        },
        {
          ...
          customRender: function (text) {
            return !text ? "" : (text.length > 7 ? text.substr(0, 6) : text)
          },
      ...
      url: {
        list: "/dbpjbxxdjb/sgztDbpjbxxdjb/list",
        delete: "/dbpjbxxdjb/sgztDbpjbxxdjb/delete",
        deleteBatch: "/dbpjbxxdjb/sgztDbpjbxxdjb/deleteBatch",
        deleteByBatch: "/dbpjbxxdjb/sgztDbpjbxxdjb/deleteByBatch",
        exportXlsUrl: "dbpjbxxdjb/sgztDbpjbxxdjb/exportXls",
        importExcelUrl: "dbpjbxxdjb/sgztDbpjbxxdjb/importExcel",
        exportTemplateUrl:"/dbpjbxxdjb/sgztDbpjbxxdjb/exportTemplateXls",
      },
      exportTemplateName: '担保品基本信息登记簿导入模板'
    }
  },
  computed: {},
  methods: {
    ...
    getQueryParams () {
      ...
      if (this.queryParam.rkrq) {
        rkMoment=moment(this.queryParam.rkrq).format("YYYYMMDD")
      }
      if (this.queryParam.ckrq) {
        ckMoment=moment(this.queryParam.ckrq).format("YYYYMMDD")
      }
