      ...
      <a-form :form="form">
        <a-tabs default-active-key="1">
          ...
          <a-tab-pane tab="背靠背评议" key="2" :forceRender="true">
            ...
            <a-divider orientation="left" style="color: #1890FF; margin-top: -20px;">评议信息列表</a-divider>
            <div>
              <a-button @click="handleAddPyxx" type="primary" style=" margin-top: 2px;width: 120px" icon="plus">新增评议</a-button>
              <!--<a-table bordered
                       ref="table"
                       size="middle"
             ...
             <pyxxlb ref="pyxxlb"/>
            <pyxx-modal ref="PyxxModal" @ok="queryPyxx()"/>
            </div>
...
<script>
  ...
  import PyxxModal from './edit/PyxxModal'
  import Pyxxlb from './Pyxxlb.vue'
  ...
  export default {
    ...
    components: { AFormItem ,JUploadOne,photoView,HzxxPhotoView,Jtxgqk,JMultiSelectTag,JSelectYgxx,
      JEllipsis,Pjsxxx,PyxxModal,Pyxxlb},
    ...
    methods: {
      ...
      queryPyxx(){
          getAction("/nh/pyxx/queryPyxx",{id: this.model.id}).then((res) => {
            if (res.success) {
              this.$refs.pyxxlb.PyxxglTable.dataSource = res.result;
            } else {
              this.$refs.pyxxlb.PyxxglTable.dataSource = [];
            }
          });
        },
      ...
      handleAddPyxx(){
        ...
        this.$refs.PyxxModal.add()
        this.$refs.PyxxModal.title = "新增背靠背评议";
        this.$refs.PyxxModal.disableSubmit = false;

      },
      handleEditPyxx(record){
        this.$refs.PyxxModal.edit(record)
        this.$refs.PyxxModal.title = "编辑背靠背评议";
        this.$refs.PyxxModal.disableSubmit = false;
      },

      handleDeletePyxx: function (id) {
        var that = this;
        deleteAction("/nh/pyxx/deletePyxx", {id: id}).then((res) => {
          if (res.success) {
            that.$message.success(res.message);
            that.queryPyxx();
          } else {
