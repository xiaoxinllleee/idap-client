  ...
  <a-card :bordered="false">
    ...
    <div class="table-operator">
     ...
     <!-- <a-button type="primary" icon="download" @click="handleExportXls('农户信息管理')">导出</a-button>-->
      ...
      <a-button @click="searchQuery" type="primary" icon="search">查询</a-button>
      <a-button @click="searchReset" type="primary" icon="reload" style="margin-left: 8px;">重置</a-button>
      <a-button type="primary" icon="download" :loading="exportLoading" style="margin-left: 8px;" @click="handleExportXls('其他自然人采集信息')">导出采集信息</a-button>
      ...
      <a-table bordered
        ...
        <!--  <a @click="handleDetail(record)">详情</a>-->
         ...
         <!-- <a-divider type="vertical"/>-->
          <a @click="handlemxExportXls('其他自然人基本信息',record)">导出</a>
          <a-divider type="vertical"/>
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              ...
              <a-menu-item >
                  <a v-if="record.sfypyjl==1" href="javascript:;" @click="handlepjsxspbPrint(record)">评级授信审批表</a>
               </a-menu-item>
            </a-menu>
          </a-dropdown>
        </span>
    ...
    <qtzrrjbxx-xq-modal ref="QtzrrjbxxXqModal" />
    <PrintYX ref="Print" v-if="qybm == '350'"/>
    <Print ref="Print" v-else/>
    <qtzrrpjsxspb-print ref="pjsxPrint"/>
    <edit-qtzrrjbxx-modal  ref="EditModalForm" @ok="modalFormOk" />
...
<script>
  ...
  import EditQtzrrjbxxModal from './modules/EditQtzrrjbxxModal'
  import QtzrrjbxxXqModal from './modules/QtzrrjbxxXqModal'
  import Print from './modules/Print'
  import PrintYX from './modules/PrintYX'
  import QtzrrpjsxspbPrint from './modules/QtzrrpjsxspbPrint'
  ...
  export default {
    ...
    components: {
      JEllipsis,
      NhjbxxModal,Print,PrintYX,
      JInput,JTreeSelect,QtzrrjbxxXqModal,QtzrrpjsxspbPrint,EditQtzrrjbxxModal
    },
    data () {
      return {
        description: '其他自然人管理页面',
        isAll: false,
        sszh:'',
        sfktsjyxdy: 1,
        sfjsejyxdy: true,
        ...
        url: {
          ...
          delete: "/qtzrr/qtzrrjbxx/delete",
          deleteBatch: "/qtzrr/qtzrrjbxx/deleteBatch",
          exportXlsUrl: "qtzrr/qtzrrjbxx/exportXls",
          importExcelUrl: "qtzrr/qtzrrjbxx/importExcel",
          qtzrrmxexportXlsUrl:"qtzrr/qtzrrjbxx/qtzrrmxexportTemplateXls",
        },
      }
    },
    computed: {
    ...
    created(){
      ...
      if (this.qybm == '350') {
        this.url.exportXlsUrl = "qtzrr/qtzrrjbxx/exportCjxxXls"
      }
    },
    methods: {
      ...
      handleEdit:function(record){
        if(record.hhbm) {
          getAction("/qtzrr/qtzrrjbxx/queryHzxxByHhbm",{ hhbm: record.hhbm}).then((res) => {
            if (res.success) {
      ...
      initData() {
        this.loading=true;
        putAction("/qtzrr/qtzrrjbxx/extract").then((res) => {
          if (res.success) {
            alert("提取成功")
          } else {
            alert("提取失败")
      ...
      handlepjsxjdmxEdit: function(record) {
        this.$refs.QtzrrjbxxXqModal.edit(record);
        this.$refs.QtzrrjbxxXqModal.title = "详情";
        this.$refs.QtzrrjbxxXqModal.disableSubmit = true;
      },
      ...
      handlemxExportXls(fileName,record){
        ...
        downFile(this.url.qtzrrmxexportXlsUrl,param).then((data)=>{
          if (!data) {
