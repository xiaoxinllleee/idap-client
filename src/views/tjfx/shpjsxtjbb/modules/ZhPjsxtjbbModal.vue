<template>
  <a-modal
    ...
    <div style="margin-bottom: 10px;">
      <a-button icon="download" type="primary" :loading="exportLoading" @click="handleExportXls('商户支行评级授信统计明细')">导出</a-button>
    </div>
    <a-table ref="table"
             size="middle"
             bordered
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          list: "/shpjsx/zh/tjfxZhpjsxtjbbSh/queryPageListMx",
          exportXlsUrl: "/shpjsx/zh/tjfxZhpjsxtjbbSh/exportShPjsxMxXls",
        },
        type:'',
      }
    },
    ...
    methods: {
      init(record,type){
        if (type=='1'){
          this.title='商户支行评级授信-评级户数详情'
        }else{
          this.title='商户支行评级授信-授信户数详情'
        }
      ...
      loadData () {
        console.log(this.record.sszh+"loadData")
        let params = {
          'sszh':this.record.sszh,
