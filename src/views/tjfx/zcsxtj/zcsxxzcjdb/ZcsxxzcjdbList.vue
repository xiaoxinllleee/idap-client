<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="行政村">
              <a-input placeholder="请输入行政村" v-model="queryParam.wgbh"></a-input>
            </a-form-item>
          </a-col>
        ...
        <template v-if="toggleSearchStatus">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="责任人">
              <a-input placeholder="请输入责任人" v-model="queryParam.zkhjl"></a-input>
            </a-form-item>
          </a-col>
    ...
    <div class="table-operator">
      <a-button icon="download" :loading="exportLoading" @click="handleExportXls('整村授信行政村进度表')">导出</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot='hnkd_drhs' slot-scope='hnkdDrhs,record'>
          <a @click="detailInfo(record,'1','cun',null)">{{ hnkdDrhs }}</a>
        </span>
        <span slot='hnkd_dhhf' slot-scope='hnkdDhhfhs,record'>
            <a @click="detailInfo(record,'2','cun',null)">{{ hnkdDhhfhs }}</a>
        </span>
        <span slot='hnkd_qykhs' slot-scope='hnkdQyhs,record'>
            <a @click="detailInfo(record,'3','cun',null)">{{ hnkdQyhs }}</a>
        </span>
        <span slot='dkht_xz' slot-scope='dkhtXzkhs,record'>
          <a @click="detailInfo(record,'4','cun',null)">{{ dkhtXzkhs }}</a>
        </span>
        <span slot='dkht_byxz' slot-scope='dkhtJsyxzkhs,record'>
            <a @click="detailInfo(record,'4','cun','dkht_byxz')">{{ dkhtJsyxzkhs }}</a>
        </span>
        <span slot='dkht_bzxz' slot-scope='dkhtJszxzkhs,record'>
            <a @click="detailInfo(record,'4','cun','dkht_bzxz')">{{ dkhtJszxzkhs }}</a>
        </span>
        <span slot='dkht_nqkh' slot-scope='dkhtNqkhs,record'>
            <a @click="detailInfo(record,'4','cun','dkht_nqkh')">{{ dkhtNqkhs }}</a>
        </span>
        <span slot='yx_xz' slot-scope='dkyxXzkhs,record'>
            <a @click="detailInfo(record,'5','cun','yx_xz')">{{ dkyxXzkhs }}</a>
        </span>
        <span slot='yx_byxz' slot-scope='dkyxJsyxzkhs,record'>
            <a @click="detailInfo(record,'5','cun','yx_byxz')">{{ dkyxJsyxzkhs }}</a>
        </span>

        <span slot='yxyx_xz' slot-scope='dkyxXzyxyxkhs,record'>
            <a @click="detailInfo(record,'5','cun','yxyx_xz')">{{ dkyxXzyxyxkhs }}</a>
        </span>
        <span slot='yxyx_byxz' slot-scope='dkyxJsyxzyxyxkhs,record'>
            <a @click="detailInfo(record,'5','cun','yxyx_byxz')">{{ dkyxJsyxzyxyxkhs }}</a>
        </span>
    ...
    <!-- 表单区域 -->
    <zcsxxzcjdb-modal ref="modalForm" @ok="modalFormOk"></zcsxxzcjdb-modal>
    <zcsxmx-modal ref='zcsxmx'></zcsxmx-modal>
  </a-card>
</template>

<script>
  import ZcsxxzcjdbModal from './modules/ZcsxxzcjdbModal'
  ...
  export default {
    ...
    components: {
      ZcsxxzcjdbModal,JTreeSelect,zcsxmxModal
    },
    data () {
      return {
        description: '整村授信行政村进度表管理页面',
        // 表头
        defaultMonthOpera: '',
        dateValue: '',
          ...
          },
          {
            ...
            children: [
              ...
              },
              {
                ...
                align:"center",
                dataIndex: 'dkhtNqkhs',
                scopedSlots: { customRender: 'dkht_nqkh' }
              },
              {
                title: '新增签约额度汇总(万元)',
                align:"center",
		...
		url: {
          list: "/zcsxxzcjdb/zcsxxzcjdb/list",
          delete: "/zcsxxzcjdb/zcsxxzcjdb/delete",
          deleteBatch: "/zcsxxzcjdb/zcsxxzcjdb/deleteBatch",
          exportXlsUrl: "zcsxxzcjdb/zcsxxzcjdb/exportXls",
          importExcelUrl: "zcsxxzcjdb/zcsxxzcjdb/importExcel",
       },
    }
  },
    created() {
  ...
  computed: {
    ...
    methods: {
      getCurrentMonth() {
        let curDate =''
        var params ={}
        getAction("/zcsxxzcjdb/zcsxxzcjdb/maxDate",params).then((res) => {
          console.log(res)
          if (res.success) {
             curDate= res.result;
          }
