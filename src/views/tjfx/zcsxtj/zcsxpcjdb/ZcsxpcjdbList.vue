<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="统计范围">
              <a-select placeholder="请选择统计范围" v-model="queryParam.nf" :defaultValue="dateValue">
    ...
    <div class="table-operator">
      <a-button icon="download" :loading="exportLoading" @click="handleExportXls('整村授信批次进度表')">导出</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot='hnkd_drhs' slot-scope='hnkdDrhs,record'>
          <a @click="detailInfo(record,'1','pc',null)">{{ hnkdDrhs }}</a>
        </span>
        <span slot='hnkd_dhhf' slot-scope='hnkdDhhfhs,record'>
            <a @click="detailInfo(record,'2','pc',null)">{{ hnkdDhhfhs }}</a>
        </span>
        <span slot='hnkd_qykhs' slot-scope='hnkdQyhs,record'>
            <a @click="detailInfo(record,'3','pc',null)">{{ hnkdQyhs }}</a>
        </span>
        <span slot='dkht_xz' slot-scope='dkhtXzkhs,record'>
          <a @click="detailInfo(record,'4','pc',null)">{{ dkhtXzkhs }}</a>
        </span>
        <span slot='dkht_byxz' slot-scope='dkhtJsyxzkhs,record'>
            <a @click="detailInfo(record,'4','pc','dkht_byxz')">{{ dkhtJsyxzkhs }}</a>
        </span>
        <span slot='dkht_bzxz' slot-scope='dkhtJszxzkhs,record'>
            <a @click="detailInfo(record,'4','pc','dkht_bzxz')">{{ dkhtJszxzkhs }}</a>
        </span>
        <span slot='yx_xz' slot-scope='dkyxXzkhs,record'>
            <a @click="detailInfo(record,'5','pc','yx_xz')">{{ dkyxXzkhs }}</a>
        </span>
        <span slot='yx_byxz' slot-scope='dkyxJsyxzkhs,record'>
            <a @click="detailInfo(record,'5','pc','yx_byxz')">{{ dkyxJsyxzkhs }}</a>
        </span>
        <span slot='yxyx_xz' slot-scope='dkyxXzyxyxkhs,record'>
            <a @click="detailInfo(record,'5','pc','yxyx_xz')">{{ dkyxXzyxyxkhs }}</a>
        </span>
        <span slot='yxyx_byxz' slot-scope='dkyxJsyxzyxyxkhs,record'>
            <a @click="detailInfo(record,'5','pc','yxyx_byxz')">{{ dkyxJsyxzyxyxkhs }}</a>
        </span>
...
<script>
  import ZcsxpcjdbModal from './modules/ZcsxpcjdbModal'
  ...
  export default {
    ...
    components: {
      ZcsxpcjdbModal,JTreeSelect,zcsxmxModal
    },
    data () {
      return {
        description: '整村授信批次进度表管理页面',
        // 表头
        defaultMonthOpera: '',
        dateValue: '',
        show:'',
		...
		url: {
          list: "/zcsxpcjdb/zcsxpcjdb/list",
          delete: "/zcsxpcjdb/zcsxpcjdb/delete",
          deleteBatch: "/zcsxpcjdb/zcsxpcjdb/deleteBatch",
          exportXlsUrl: "zcsxpcjdb/zcsxpcjdb/exportXls",
          importExcelUrl: "zcsxpcjdb/zcsxpcjdb/importExcel",
       },
    }
  },
  computed: {
    ...
    methods: {
      getCurrentMonth() {
        let curDate =''
        var params ={}
        getAction("/zcsxpcjdb/zcsxpcjdb/maxDate",params).then((res) => {
          console.log(res)
          if (res.success) {
            curDate= res.result;
          }
