...
<!--          </a-col>-->
    ...
    <div class="table-operator">
      <a-button icon="download" @click="handleExportXls('个人走访（汇总）')" v-has="'GrzfHz:exp'">导出</a-button>
      <a-button icon="area-chart" @click="init" v-has="'nhgrzf:init'" >提取</a-button>
      <a-button icon="area-chart" @click="batchInit" v-has="'nhgrzf:batchInit'">批量提取</a-button>
    ...
    <a-modal
      title="批量提取"
      :width="800"
      :visible="batchInitVisible"
      :confirmLoading="batchInitLoading"
      @ok="batchInitOk"
      @cancel="batchInitCancel"
      okText="开始提取"
      cancelText="关闭">
      <div>
        <a-form layout="inline">
          <a-row :gutter="24">
            <a-col :span="24">
              <a-form-item label="提取区间">
                <a-range-picker v-model="batchInitDate"/>
              </a-form-item>
            </a-col>
          </a-row>
        </a-form>
      ...
      return {
       ...
       //     },
		...
		url: {
          ...
          exportXlsUrl: "/zfsjtj/khjlzfsjtj/exportXlsnhhz",
          importExcelUrl: "/zfsjtj/khjlzfsjtj/importExcel",
          init: "khjlzfyxtj/khjlZfyxtj/init",
          batchInit: "khjlzfyxtj/khjlZfyxtj/batchInit",
    },
        batchInitVisible: false,
        batchInitLoading: false,
        batchInitDate: undefined
    }
  },
  computed: {
    ...
    methods: {
      ...
      zfs(record,lx){
        ...
        console.log(record)
        console.log(lx+"lx")
        this.$refs.nhzfmxModal.init(record,lx,'GR',null,null,'1');
      },
      getQueryParams() {
        let tjyfMoment = "";
      ...
      init() {
        ...
        let params = {
          tjrq: moment(this.queryParam.tjrq).format("YYYYMMDD"),
        }
        this.loading = true
      ...
      batchInitOk() {
        console.log('batchInitOk', this.batchInitDate)

        if (this.batchInitDate === undefined || this.batchInitDate.length < 2) {
          this.$message.warning("请选择提取区间！")
          return false
        }
        let params = {
          beginDate: moment(this.batchInitDate[0]).format("YYYYMMDD"),
          endDate: moment(this.batchInitDate[1]).format("YYYYMMDD"),
        }
        console.log('params', params)
        this.batchInitLoading = true
        postAction(this.url.batchInit, params).then((res) => {
          if (res.success) {
            this.$message.success("提取成功！")
            this.batchInitCancel()
            this.loadData(1)
          } else {
            this.$message.warning("提取失败！")
          }
          ...
          this.batchInitLoading = false
        })
      },
      batchInitCancel() {
        this.batchInitVisible = false
      },
      // loadData(arg) {
      //   if(!this.url.list){
