<template>
  <a-card :bordered="false">
      ...
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <template v-if="toggleSearchStatus">
            <a-col :md="6" :sm="8"><a-form-item label="行政组" v-if="this.sfktsjyxdy==1">
            ...
            <a-col :md="6" :sm="8"><a-form-item label="是否有效走访">
              <j-dict-select-tag placeholder="请选择是否有效走访" v-model="queryParam.sfbz_ex" dictCode="sfbz"/>
            </a-form-item></a-col>
          </template>
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('单户走访数据明细统计')">导出</a-button>
...
<!--        </span>-->
        <span slot="detail" slot-scope="hhbm, record">
          <a @click="handlerDetailToGr(record, '/tjfx/zfsjmx/grsjmx/ZfsjmxGrList')">{{hhbm}}</a>
        </span>
      </a-table>
    </div>
    <!-- table区域-end -->
    ...
    <!-- 表单区域 -->
    <zfsjmxDh-modal ref="modalForm" @ok="modalFormOk"></zfsjmxDh-modal>
  </a-card>
</template>

<script>
  import ZfsjmxDhModal from './modules/ZfsjmxDhModal'
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            onFilter: (value, record) => record.sfyxzf.indexOf(value) === 0,
          },
          // {
          //   title: '操作',
          //   dataIndex: 'action',
        ...
        url: {
          list: "/tjfx.zfsjmx/dhsjmx/list",
          delete: "/tjfx.zfsjmx/dhsjmx/delete",
          deleteBatch: "/tjfx.zfsjmx/dhsjmx/deleteBatch",
          exportXlsUrl: "tjfx.zfsjmx/dhsjmx/exportXls",
          importExcelUrl: "tjfx.zfsjmx/dhsjmx/importExcel",
        },
      }
    },
    computed: {
