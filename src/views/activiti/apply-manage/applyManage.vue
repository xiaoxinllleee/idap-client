...
<template>
  <div class="search">
    ...
    <Card v-show="!showHistory">
      ...
      <Row class="operation">
        <!--功能未完善 暂时去掉新增申请<Button @click="add" type="primary" icon="md-add">新增申请</Button>-->
...
export default {
  ...
  methods: {
    ...
    getProcessList() {
      ...
      console.log(this.searchProcessForm)
    ...
    getDataList() {
      ...
      getBusinessDataList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
          console.log("返回数据"+res.result.content)
    ...
    edit(v) {
      ...
      //   return;
      // }
      if (v.procDefId.split(":")[0]=='loanDeferApproval'){
        this.$nextTick(()=>{
        ...
        console.log('this.$refs[detailModal]1', this.$refs.detailModal)
        console.log('this.hrefComponent', this.hrefComponent)
        this.$nextTick(() => {
          this.componentCreate = true
          console.log('this.hrefComponent', this.hrefComponent)
          console.log('this.$refs[detailModal]2', this.$refs.detailModal)
    ...
    apply(v) {
      ...
      this.form.id = v.id;
      console.log("提交申请时id："+v.id, v)
      this.form.procDefId = v.procDefId;
      this.form.title = v.title;
      // 加载审批人
      this.userLoading = true;
      getFirstNode(v.procDefId, {tableId: v.tableId}).then(res => {
        this.userLoading = false;
        if (res.success) {
          this.error = "";
