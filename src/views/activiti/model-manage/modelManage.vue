...
<template>
  <div class="search">
    ...
    >
      <div slot="header">
        <div class="ivu-modal-header-inner">工作流程设计</div>
...
export default {
  ...
  methods: {
    ...
    edit(v) {
      this.modelerUrl = `${window._CONFIG['domianURL']}/modeler.html?modelId=${v.id}&accessToken=${Vue.ls.get(ACCESS_TOKEN)}&time=${new Date()}`;
    ...
    showJump() {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$router.push({
           path:'/activiti/process-manage/processManage'
          });
        }
      });
    },
    export(v) {
      window.open(
        `${window._CONFIG['domianURL']}`+exportModel + v.id
      );
    },
    remove(v) {
      this.$Modal.confirm({
