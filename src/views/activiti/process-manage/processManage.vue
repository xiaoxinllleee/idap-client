...
export default {
  ...
  methods: {
    init() {
      this.deployByFileUrl =`${window._CONFIG['domianURL']}/actModel/deployByFile`
      this.getDataList();
    ...
    showJump() {
      this.$Modal.confirm({
        ...
        content: "是否立即跳转查看该模型?",
        onOk: () => {
          this.$router.push({path:'/activiti/model-manage/modelManage'});
        }
      });
    },
    showResource(type, v) {
      if (type == 0) {
        window.open(
          `${window._CONFIG['domianURL']}/actProcess/export?id=${
            v.id
          }&type=${type}`
        );
      } else if (type == 1) {
        this.viewTitle = "流程图片预览(" + v.diagramName + ")";
        this.diagramUrl = `${window._CONFIG['domianURL']}/actProcess/export?id=${
          v.id
        }&type=${type}`;
        this.viewImage = true;
      }
