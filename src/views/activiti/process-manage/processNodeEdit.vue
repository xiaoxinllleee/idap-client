        ...
        <Col :xs="24" :md="24" :lg="16" :xl="9">
          ...
          >
            ...
            <FormItem label="关联业务表" prop="businessTable">
              <Select v-model="basicForm.businessTable" placeholder="请选择关联数据库业务表" clearable>
                <Option
            ...
            <FormItem label="备注描述" prop="description">
              <Input v-model="basicForm.description" type="textarea" :rows="4"/>
            </FormItem>
            <FormItem class="br">
              <Button type="warning" @click="nextStep">
                下一步
          ...
          <Form ref="form" :model="form" :label-width="90" style="position:relative">
            ...
            <div v-show="form.type==0">
              <FormItem label="可发起人员">
                ...
                <!--<Checkbox v-model="choosePost" @on-change="clickPost" :disabled="allUser">-->
                  <!--<Icon type="md-contact" size="14" style="margin:0 2px 0 0"></Icon>-->
                  <!--<span>指定岗位</span>-->
                <!--</Checkbox>-->
                <!--<Checkbox
            ...
            <div v-show="form.type==1">
              <FormItem label="可审批人员">
                ...
                <Checkbox v-model="choosePost" @on-change="clickPost" :disabled="form.customUser">
                  <Icon type="md-contact" size="14" style="margin:0 2px 0 0"></Icon>
                  <span>指定岗位</span>
                </Checkbox>
                <!--<Checkbox
                ...
                <Checkbox v-model="chooseDynamic" @on-change="clickDynamic" :disabled="form.customUser">
                  <Icon type="md-shuffle" style="margin:0 2px 0 0"></Icon>
                  <span>动态设置</span>
                </Checkbox>
            ...
            <FormItem
              label="选择岗位"
              v-show="choosePost&&((form.type==0&&!allUser)||(form.type==1&&!form.customUser))"
            >
              <Select v-model="form.posts" multiple>
                <Option v-for="item in postList" :value="item.gwbz" :key="item.gwbz" :label="item.gwmc">
                  <span style="margin-right:10px;">{{ item.gwmc }}</span>
                </Option>
              </Select>
            </FormItem>
            ...
            <div v-show="chooseDynamic&&((form.type==0&&!allUser)||(form.type==1&&!form.customUser))">
              <FormItem label="输入类名">
                <Input v-model="form.dynamicClass" />
              </FormItem>
            </div>
...
export default {
  ...
  methods: {
    ...
    selectTree(v) {
      if (v.length > 0) {
        ...
        if (data.dynamicClass) {
          this.chooseDynamic = true
        } else {
          this.chooseDynamic = false
        }
        this.form = data;
      } else {
        this.cancelEdit();
    ...
    handelSubmit() {
      ...
      if (this.choosePost) {
        if (this.form.posts && this.form.posts.length > 0) {
          this.form.postIds = this.form.posts;
        }
      }
      if (!this.chooseDynamic) {
        this.form.dynamicClass = ''
      }
      if (this.form.type == 0) {
