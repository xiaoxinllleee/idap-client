<template>
  ...
  >
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item label="子系统" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select v-model="subSystemId" @change="onChangeSubSystem">
            <a-select-option v-for="(item,index) in subSystemList" :key="index" :value="item.id">{{ item.xtmc }}</a-select-option>
          </a-select>
        </a-form-item>
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="后台服务地址">
          <a-input placeholder="请输入后台服务地址" v-decorator="[ 'urlServer', {}]" :readOnly="disableSubmit"/>
        </a-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      loadTree(){
        var that = this;
        queryTreeList({subSystemId: this.subSystemId}).then((res)=>{
          if(res.success){
      ...
      add (item) {
        // 默认值
        let record = Object.assign({status:'1',permsType:'1',route:true}, item)
        this.edit(record);
      },
      edit (record) {
        ...
        this.subSystemId = record.subSystemId
        ...
        this.visible = true;
        this.loadTree();
        let fieldsVal = pick(this.model,'name','perms','permsType','component','url','sortNo','menuType','status','urlServer');
        this.$nextTick(() => {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.model.subSystemId = this.subSystemId
            console.log('this.model', this.model, values)
      ...
      onChangeSubSystem(e) {
        console.log('onChangeSubSystem', e)
        //重新获取上级菜单
        this.loadTree()
        if (this.localMenuType == 0 || this.localMenuType == 1) {
