...
<!--                             show-search :showSearch='true' :treeDefaultExpandAll='true' :filterTreeNode='true' :isAll='false' />-->
          ...
          <a-col :xl='6' :lg='7' :md='8' :sm='24'>
            <a-form-item label='一级科目'>
              <a-input hidden placeholder='一级科目号' v-model='queryParam.subjectNo1'></a-input>
          ...
          <a-col :xl='6' :lg='7' :md='8' :sm='24'>
            <a-form-item label='二级科目'>
<!--              <a-input placeholder='请输入二级科目号' v-model='queryParam.subjectNo2'></a-input>-->
              <j-dict-select-tag placeholder='请选择' v-model='queryParam.subjectNo2' dict-code='jylrhs:jylrhs_kmsz,subject_name2,subject_no2' disabled>
              </j-dict-select-tag>
            </a-form-item>
          </a-col>
...
<!--          </a-col>-->
    ...
    <div class='table-operator'>
      <a-button @click='handleAdd' type='primary' icon='plus' v-has="'JylrhsKmszJg:add'">新增</a-button>
      <a-button type='primary' icon='download' :loading='exportLoading' @click="handleExportXls('经营利润核算（机构科目设置）')" v-has="'JylrhsKmszJg:export'">导出
      </a-button>
      <a-button icon="import" @click="excelImport" v-has="'JylrhsKmszJg:import'">导入</a-button>
...
<!--      </div>-->
      ...
      <a-table
        ...
        <span slot='action' slot-scope='text, record'>
          <a @click='handleEdit(record)' v-has="'JylrhsKmszJg:edit'">编辑</a>

          <a-divider type='vertical' v-has="'JylrhsKmszJg:delete'"/>
          <a-dropdown v-has="'JylrhsKmszJg:delete'">
    ...
    <!-- 表单区域 -->
    <jylrhsKmszJg-modal ref='modalForm' @ok='modalFormOk'></jylrhsKmszJg-modal>
...
<script>
import JylrhsKmszJgModal from './modules/JylrhsKmszJgModal'
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/jylrhs/csgl/jgkmsz/list',
        delete: '/jylrhs/csgl/jgkmsz/delete',
        deleteBatch: '/jylrhs/csgl/jgkmsz/deleteBatch',
        exportXlsUrl: 'jylrhs/csgl/jgkmsz/exportXls',
        importExcelUrl: 'jylrhs/csgl/jgkmsz/importExcel',
        exportTemplateUrl: 'jylrhs/csgl/jgkmsz/exportTemplateXls',
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    excelImport() {
      ...
      this.exportTemplateName = '经营利润核算（机构科目设置）导入模板' + '_' + moment(new Date()).format('YYYYMMDDHHmmss')
    ...
    selectOK: function (data) {
      ...
      this.queryParam.subjectNo1 = data[0].subjectNo1
      this.queryParam.subjectNo2 = data[0].subjectNo2
    },
