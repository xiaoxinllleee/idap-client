        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="执行状态">
              <j-dict-select-tag placeholder="请选择执行状态" v-model="queryParam.zxzt" dict-code="job_status" />
            </a-form-item>
          </a-col>
...
<script>
  ...
  import {extract,updatezt} from '@/api/api'
  ...
  computed: {
    ...
    methods: {
      ...
      handleEdit(record){
        var that=this;
        console.log(record);
        updatezt({tjrq:record.tjrq,rwid: record.rwid}).then((res) => {
          if (res.success) {
            that.$message.success(res.message);
            that.loadData();
            extract({tjrq:record.tjrq,rwid: record.rwid, spname: this.spname,}).then((res) => {
              if (res.success) {
                that.loadData();
              } else {
              }
