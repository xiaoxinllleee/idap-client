    ...
    <Collapse simple class="collapse">
      <Panel name="1">
        ...
        <p slot="content">
          ...
          >
            <Tooltip placement="top" :content="item.username">{{item.realname}}</Tooltip>
          </Tag>
        </p>
      </Panel>
    </Collapse>
    <Drawer title="选择用户" closable v-model="userModalVisible" width="800" draggable>
      <Form ref="searchUserForm" :model="searchUserForm" inline :label-width="55">
        <Form-item label="真实姓名" prop="realname">
          <Input
            type="text"
            v-model="searchUserForm.realname"
            clearable
            placeholder="请输入真实姓名"
            style="width: 200px"
          />
...
<script>
import { getUserListData } from "@/api/activiti";
  ...
  methods: {
    ...
    chooseUser(v) {
      ...
      if (flag) {
        ...
        this.$emit("on-change", this.selectUsers);
        this.$Message.success(`添加用户 ${v.realname} 成功`);
      }
    },
    clearSelectData() {
      this.selectUsers = [];
