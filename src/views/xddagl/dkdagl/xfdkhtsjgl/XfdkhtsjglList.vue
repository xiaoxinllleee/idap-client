<template>
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="客户类型">
                <j-dict-select-tag placeholder="请选择客户类型" v-model="queryParam.khlx" dictCode="xddagl_khlx"></j-dict-select-tag>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
            ...
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="贷款品种(补充)">
                <j-dict-select-tag placeholder="请选择贷款品种(补充)" v-model="queryParam.dkpzbc" dictCode="dkzlbc"></j-dict-select-tag>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="担保方式">
                <j-dict-select-tag placeholder="请选择担保方式" v-model="queryParam.dbfs" dictCode="gljydbfs"></j-dict-select-tag>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
            ...
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="是否上传档案">
                <j-dict-select-tag placeholder="请选择是否上传档案" v-model="queryParam.sfscda" dictCode="sfbs"></j-dict-select-tag>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="贷款类型">
                <j-dict-select-tag placeholder="请选择贷款类型" v-model="queryParam.lx" dictCode="xddagl_dklx"></j-dict-select-tag>
              </a-form-item>
            </a-col>
          </template>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div class="table-operator">
      <a-button icon="download" :loading="exportLoading" @click="handleExportXls('新放贷款合同数据管理')" v-has="'xfdkhtsj:exportXls'">导出</a-button>
      <a-button icon="import" @click="excelImport" v-has="'xfdkhtsj:import'">导入</a-button>
...
<!--        </span>-->
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-has="'xfdkhtsj:edit'">上传</a>
          <a-divider type="vertical" />
          <a @click="handleView(record)" v-has="'xfdkhtsj:view'">查看</a>
          <a-divider type="vertical" />
          <a @click="scewm(record)" v-has="'dkhtsjgl:scewm'">生成二维码</a>
        </span>
    ...
    <!-- 表单区域 -->
    <xfdkhtsjgl-modal ref="modalForm" @ok="modalFormOk"></xfdkhtsjgl-modal>
    <transfer-modal ref="transferModal" @close="transferClose"></transfer-modal>
    <excel-import ref="excelImportModal" @ok="importComplete"/>
    <scrwm-modal ref="scrwmModal" />
  </a-card>
</template>

<script>
  import XfdkhtsjglModal from './modules/XfdkhtsjglModal'
  import TransferModal from '@views/xddagl/dkdagl/cldkhtsjgl/modules/TransferModal'
  ...
  import ScrwmModal from '@views/xddagl/dkdagl/dkdahtsjgl/modules/ScrwmModal'
  ...
  export default {
    ...
    components: {
      XfdkhtsjglModal,JTreeSelect,JEllipsis,ExcelImport,TransferModal,ScrwmModal
    },
    data () {
      return {
        description: '新放贷款合同数据管理管理页面',
        // 表头
        columns: [
          {
            title: '#',
		...
		url: {
          list: "/xfdkhtsjgl/xfdkhtsjgl/list",
          delete: "/xfdkhtsjgl/xfdkhtsjgl/delete",
          deleteBatch: "/xfdkhtsjgl/xfdkhtsjgl/deleteBatch",
          exportXlsUrl: "xfdkhtsjgl/xfdkhtsjgl/exportXls",
          importExcelUrl: "xfdkhtsjgl/xfdkhtsjgl/importExcel",
          exportTemplateUrl: "/xfdkhtsjgl/xfdkhtsjgl/exportTemplateXls",
          scewmpd: "/dkdahtsjgl/dkdahtsjgl/scewmpd"
       },
        exportTemplateName: '新放贷款合同数据管理导入模板'
    }
  },
  computed: {
    ...
    methods: {
      ...
      scewm(record){
        putAction(this.url.scewmpd, record).then((res) => {
          if (res.success) {
            this.$refs.scrwmModal.init(record);
          } else {
