...
<script>
  ...
  import { getImportCoustmerNumber } from '../../api/api'
  import store from '@/store/'

  const indexKey = '/dashboard/HomePage'
  ...
  export default {
    ...
    data() {
      return {
        birthdayPeopleNumber:0,
        birthdayPeopleNumberzhtxs:0,
        birthdayPeopleNumbergrtxs:0,
        title:window._CONFIG['title'],
        pageList: [],
        linkList: [],
        activePage: '',
        menuVisible: false,
    ...
    watch: {
      '$route': function(newRoute) {
        ...
        // update-end-author:taoyan date:20200211 for: TASK #3368 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
          ...
          // 2023-4-11 16:41:16 暂时注释，不知道强制刷新的意义。此处强制刷新会导致点击打开新的菜单时，已打开页面的缓存失效。
        ...
        // let storeKey = 'route:title:' + newRoute.path
        // console.log('storeKey', storeKey)
        // this.$ls.set(storeKey, newRoute.meta.title)
      },
    ...
    methods: {
      ...
      async getBirthdayNumberCount () {
        await  getImportCoustmerNumber().then((data) => {
          this.birthdayPeopleNumberzhtxs=data.result.zhtxs;
          this.birthdayPeopleNumbergrtxs=data.result.grtxs;
          //时间格式化
        })
      ...
      openNotification () {
        ...
        const btn = (
          <Button type="primary" size="small" onClick={() => {
            this.linkBirthdayRemindPage(key)
          }}>
            点击跳转
          </Button>
        )
        ...
        this.$notification.warn({
          ...
          btn,
          description:
            '未来30天，支行有' + this.birthdayPeopleNumberzhtxs + '个重要客户生日;'+'您已锁定重要客户生日'+this.birthdayPeopleNumbergrtxs+'个',
          duration: 10,
          placement: 'topRight'
        })
      },

      linkBirthdayRemindPage (key) {
        this.$notification.close(key)
        this.$router.push({ path: '/tjfx/srtx' })
