...
<template>
  <a-spin :spinning="loading">
    ...
    <a-row type="flex">
      ...
      <a-col>
        <!-- 操作按钮 -->
        <div v-if="actionButton || deleteButton" class="action-button">
          <a-button v-if="actionButton" type="primary" icon="plus" @click="handleClickAdd">{{AddButtonName}}</a-button>
          <span class="gap"></span>
              ...
              "
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <label :key="i" v-else-if="col.type === formTypes.textarea">
                      <a-tooltip
                        :id="id"
                        placement="top"
                        :title="(tooltips[id] || {}).title"
    ...
    methods: {
      ...
      handleInputCommono(target, index, row, column) {
        ...
        if (`${dataset.inputNumber}` === 'true') {
          ...
          let replace = value.replace(/[^-?\d+\.?\d*$]/g, '')
          if (value !== replace) {
            change = false
            value = replace
      ...
      elemValueChange(type, rowSource, columnSource, value) {
        ...
        this.$emit('valueChange', { type, row, column, value, target: this ,newDataSource:this.inputValues})
      },
